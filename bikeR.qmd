---
title: "Bike Project Documentation"
...
```{r Variaveis, echo=FALSE, message=FALSE, warning=FALSE}
prop <- .01
```
...
Also, it is important to notice the whole database contains `r formatC(nrow(trip_raw), digits = 0, format = 'f', big.mark = '.', decimal.mark = ',')` observations and this can demand extra processing time without necessity. So I decide to use the `dplyr` R package `slice_sample()` function to adequately crop the table in `r round(prop*100, 0)`% of its total.
...
All the transformation and cleaning process is documented above. Nonetheless it was used `r round(prop*100, 0)`% sample to do the exercise regarding it would not matter for the final results.
...
In this case it can be stated that the members use in week busy days is consistently higher than on the weekends. This suggests the hypothesis of use for work. The hourly behavior in the @fig-usertype-day_of_week-hour-hist shows the conclusion by the peaks (rush hours) in business days for member users, it is really mainly used for work.

```{r usertype-day_of_week-hour-hist}
#| label: fig-usertype-day_of_week-hour-hist
#| fig-cap: "User type vs day_of_week by hour"
...
```{r member-dow-gender-facet-wrap}
...
trip_for_analysis %>% 
  ...
  ggplot(aes(day_of_week, age, fill = gender)) +
  # geom_jitter(size = 2, alpha = .6) +
...
```{r usertype-daytype-ride-facet-q13}
...
For the @fig-usertype-ride-scat-all it can be extracted the fact there is no significant correlation between age and ride length (smooth lines almost parallel) and also that it is the same for both gender.
...
Average Age: `r round(mean(trip_for_analysis$age[trip_for_analysis$usertype == "member"]), 1)` years.
...
**Conclusion:** casual users do longer trips and mostly on weekends. By the other hand member users do shorter trips mainly on business days and mainly for work. *Maybe it is an idea to identify which users within the casual have the same members pattern and make a direct campaign as bellow filter exemplifies. At @tbl-casual-similar-member, it is shown as simple model using the members group, but it can be logistic modeled do determine the best ROC curve to use as a next step*.
